...
def main(_):
  ...
  # Problem.
  with tf.variable_scope("problem", partitioner=tf.min_max_variable_partitioner(max_partitions=2, min_slice_size=100)):
    problem, net_config, net_assignments = util.get_config(FLAGS.problem)
    loss = problem()
  global_step = tf.Variable(0, dtype=tf.int64, trainable=False)
  # Optimizer setup.
  var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='problem/mlp/linear_0')
  var_list2 = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='problem/mlp/linear_1')

  print(var_list)
  #adam_opt = tf.train.GradientDescentOptimizer(FLAGS.learning_rate)
  adam_opt = tf.train.AdamOptimizer(FLAGS.learning_rate)
  opt = adam_opt.minimize(loss, var_list=var_list)
  ...
  opt2 = optimizer.minimize(loss, global_step = global_step, unroll_len=FLAGS.unroll_len, var_list=var_list2)

  opt = tf.group(*[opt, opt2])
  if FLAGS.mode == 1:
    print('use adam opt')
  else:
    print('use l2l opt')
